...
FoxiDB.db = {
  ...
  open: function db_open(callback) {
    ...
    request.onerror = (function onDatabaseError() {
  ...
  upgrade: function db_upgrade() {
    ...
    if (upgradeFrom < 1) {
      db.createObjectStore('imgCache', { keyPath: 'uri' });
    }
  },
  ...
  createImage: function db_createImage(uri, size, data) {
    ...
    transaction.onerror = function dbTransactionError() {
...
window.clearImages = FoxiDB.clearImages.bind(FoxiDB);
export default FoxiDB;
